...
let json5: typeof Bun.JSON5;

if (typeof globalThis.Bun !== "undefined") {
	json5 = Bun.JSON5;
} else {
	const { default: json } = await import("json5");
	json5 = json;
}
...
function _for(
	...
	if (!obj) return;
	for (const [key, value] of Object.entries(obj)) fn(key, value);
}
...
function loadSuglite(app: FalconFrame, suglite: Config) {
	...
	if (map) {
		_for(map.get, (key, value) =>
			app.get(key, (req, res) => res.sendFile(value)),
		);
		_for(map.dir, (key, value) =>
			app.static(removeLeadingHyphens(key), value, {
				errorIfDirNotFound: false,
			}),
		);
		_for(map.redirect, (key, value) =>
			app.get(key, (req, res) => res.redirect(value)),
		);
	}
}
